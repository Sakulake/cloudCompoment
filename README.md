...
# AOP
面向切面编程。逻辑纵向升入的时候，添加一些业务无关的、通用的逻辑。
利用代理模式可以在切点处织入增强逻辑。
## 代理
### 静态代理
在编译前就确定了代理类和被代理类的关系。 （利用组合的关系）

### 动态代理
在运行中动态生成代理类字节码，生成代理类对象。


#### jdk动态代理
被代理类需要继承一个接口，生成的代理类也继承该接口，通过反射调用被代理类方法

切面实现 InvocationHandler接口

#### cglib动态代理
运用一个字节码工具，生成字节码。实际是被代理类的子类。所以注意final方法。

切面实现 MethodInterceptor接口

##Spring AOP

@EnableAspectJAutoProxy 开启动态代理，注解中向容器中注入一个后置处理器。在逻辑bean和增强bean都创建后，判断逻辑bean是否需要增强。
然后创建代理工厂，根据策略使用jdk或者cglib动态生成代理bean。